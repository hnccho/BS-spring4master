...
public class FixedDepositWSAsyncClient {
	...
	private static Logger logger = Logger.getLogger(FixedDepositWSAsyncClient.class);
	...
	private static void getFixedDepositList(AsyncRestTemplate restTemplate)
		...
		ListenableFuture<ResponseEntity<List<FixedDepositDetails>>> futureResponseEntity 
				= restTemplate.exchange("http://localhost:8080/ch12-webservice/fixedDeposits",
										HttpMethod.GET, requestEntity, typeRef);
		futureResponseEntity.addCallback(
			new ListenableFutureCallback<ResponseEntity<List<FixedDepositDetails>>>() {
	...
	private static void openFixedDeposit(AsyncRestTemplate restTemplate) {
		...
		futureResponseEntity.addCallback(
			new ListenableFutureCallback<ResponseEntity<FixedDepositDetails>>() {
